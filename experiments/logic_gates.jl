# logic_gates.jl — 2-input logic gates on an 8-bacteria Y (T) topology.
#
#   Input_A(1) — n_LA(2) — Junction(5) — Output(6) — n_trunk(7) — Input_0(8)
#                                  |
#               Input_B(3) — n_RB(4)
#
# The target gate (:XOR, :AND, :OR, …) is passed as ARG 1. During training the
# Output is teacher-forced on positive trials; during test S is frozen and the
# network must reproduce the gate on its own.
#
# Usage: julia --project=. experiments/logic_gates.jl [GATE] [BATCH_ID]
#        BIOKAN_KTRANSLS (default 0.04) and BIOKAN_SEED (default 43) tune the run.
...
println("=== Test XOR — topologie Y (8 bactéries SSA) — lancé le $(Dates.format(now(), "yyyy-mm-dd HH:MM:SS")) ===\n")
flush(stdout)

# 1. Model parameters (θ₀ index → name given inline)
r = 1.0
K_TRANSL_S = parse(Float64, get(ENV, "BIOKAN_KTRANSLS", "0.04"))  # config A=0.04, B=0.06 (injectable via BIOKAN_KTRANSLS)
θ₀ = Float64[
    15.0,   # 1  n
    2.0,    # 2  m
    1.0,    # 3  l
    ...
    0.15,   # 25 k_transl_M
    K_TRANSL_S*r,  # 26 k_transl_S (config A=0.04 / B=0.06 via BIOKAN_KTRANSLS)
...
# Topologie T : B1 et B3 branchés directement sur B5, pas de connexion entre eux
spacing = 1e-5
d_max   = 1.2e-5   # < 2s → B1-B3 non connectés, toutes les arêtes en T gardées
R_cell  = 0.5e-6
dt      = 1.0
...
lambda_C = 0.50 * spacing   # C, O (même que test_non_spike)
...
# 3. CONSTRUCTION DU RÉSEAU — topologie en T avec interneurones
#
# Positions (s = spacing) :
#
#   1:Input_A  (-2s, 0) — 2:n_LA  (-s, 0) ↘
#                                            5:Junction (0,-s) — 6:Output (0,-2s) — 7:n_trunk (0,-3s) — 8:Input_0 (0,-4s)
#   3:Input_B  (+2s, 0) — 4:n_RB  (+s, 0) ↗
#
# Distances clés :
#   B1-B2 = B3-B4 = s            ✓ connectés
#   B2-B5 = B4-B5 = s            ✓ connectés
#   B2-B4 = 2s > d_max           ✗ pas connectés (clé du XOR)
#   B5-B6 = B6-B7 = B7-B8 = s   ✓ connectés
#
s  = spacing
...
net = BioKan.BioNetwork(d_max, 8)
b1 = BioKan.Bacterium(1, [-2*s,   0.0], circuit, params_dict, u0_dict; mode=:ssa)  # Input_A
b2 = BioKan.Bacterium(2, [  -s,   0.0], circuit, params_dict, u0_dict; mode=:ssa)  # n_LA
b3 = BioKan.Bacterium(3, [+2*s,   0.0], circuit, params_dict, u0_dict; mode=:ssa)  # Input_B
b4 = BioKan.Bacterium(4, [  +s,   0.0], circuit, params_dict, u0_dict; mode=:ssa)  # n_RB
b5 = BioKan.Bacterium(5, [  0.0,  0.0], circuit, params_dict, u0_dict; mode=:ssa)  # Junction
b6 = BioKan.Bacterium(6, [  0.0,   -s], circuit, params_dict, u0_dict; mode=:ssa)  # Output
b7 = BioKan.Bacterium(7, [  0.0, -2*s], circuit, params_dict, u0_dict; mode=:ssa)  # n_trunk
b8 = BioKan.Bacterium(8, [  0.0, -3*s], circuit, params_dict, u0_dict; mode=:ssa)  # Input_0
...
BioKan.set_species!(net.nodes[1], :S,  150.0)  # Input_A
BioKan.set_species!(net.nodes[2], :S,  336.0)  # n_LA
BioKan.set_species!(net.nodes[3], :S,  150.0)  # Input_B
BioKan.set_species!(net.nodes[4], :S,  292.0)  # n_RB
BioKan.set_species!(net.nodes[5], :S,  67.0)  # Junction
BioKan.set_species!(net.nodes[6], :S,  150.0)  # Output
BioKan.set_species!(net.nodes[7], :S,  148.0)  # n_trunk
BioKan.set_species!(net.nodes[8], :S,  150.0)  # Input_0
...
# D_diff et C_diff : délai 150s (signal d'activation lent)
# E_ext_diff et O_diff : instantanés (enseignement/inhibition)
...
# Buffer circulaire pour le délai fixe (150 s avec dt=1)
delay_steps  = 900
delay_buffer = zeros(Float64, n_bac, n_species, delay_steps)
delay_ptr    = 1

# 4. PROTOCOLE — PORTE LOGIQUE
SEED = parse(Int, get(ENV, "BIOKAN_SEED", "43"))   # graine injectable via BIOKAN_SEED (défaut 43)
Random.seed!(SEED)
logical_gate      = length(ARGS) >= 1 ? Symbol(ARGS[1]) : :TRUE
                          # :FALSE, :AND, :A_AND_NOT_B, :A, :NOT_A_AND_B, :B, :XOR, :OR,
                          # :NOR, :XNOR, :NOT_B, :A_OR_NOT_B, :NOT_A, :NOT_A_OR_B, :NAND, :TRUE
n_epochs          = 150 # nombre d'epochs d'entraînement
n_test_epochs     = 50   # nombre d'epochs de test (S gelé)
n_pause_epochs    = 0  # epochs de silence entre entraînement et test
max_density       = 120.0
...
t_refrac      = 5000.0
t_spike_emit  = 2000.0
t_spike_emit_E = 200.0   # durée courte pour E_ext_diff → directionnalité temporelle
...
freeze_s_mean  = true   # true → S gelé = moyenne sur les n_s_mean derniers epochs d'entraînement
                        # false → S gelé = dernière valeur au moment du gel
n_s_mean       = 50    # nombre d'epochs utilisés pour la moyenne (si < n_epochs, prend le min)

# Quantité injectée par step pendant l'émission (entier pour SSA)
amp_D_diff     = 3200
...
t_o_inhib      = 5500.0   # durée d'inhibition par O (in silico), en secondes
steps_o_inhib  = round(Int, t_o_inhib / dt)

output_silent_O  = true   # true → B6 émet O quand il ne spike pas (pendant entraînement uniquement)
input_silent_O   = true   # true → B1/B3 émettent O quand leur input est absent
input_silent_C   = true  # true → Input_A (B1) et Input_B (B3) n'émettent pas C_diff lors d'un spike
...
s_hist_idx   = findfirst(==(:S), species_names)   # index dans history (par species_names)
...
emit_Eint_counter = zeros(Int, n_bac)   # durée fixe pour E_int (sans sigma)
emit_C_counter    = zeros(Int, n_bac)   # durée fixe pour C_diff (sans sigma)
emit_amp_D_vec   = fill(amp_D_diff, n_bac)
o_inhib_counter = zeros(Int, n_bac)   # compte à rebours d'inhibition par O (in silico)
s_zero_counter = zeros(Int, n_bac)
spike_logs     = [Float32[] for _ in 1:n_bac]
cancelled_logs = [Float32[] for _ in 1:n_bac]   # spikes annulés par O

valid_nodes = [net.nodes[i] for i in 1:n_bac]
node_labels = ["B1=Input_A", "B2=n_LA", "B3=Input_B", "B4=n_RB",
               "B5=Junction", "B6=Output", "B7=n_trunk", "B8=Input_0"]

s_frozen    = zeros(Float64, n_bac)
in_freeze   = false
freeze_idx  = 1

# bi=1=Input_A, bi=2=Input_B, bi=4=Output, bi=6=Input_0
forced_only  = Set([1, 3, 6, 8])   # restreints pendant entraînement
inputs_only  = Set([1, 3, 8])      # Input_A, Input_B, Input_0 : bloqués aussi en test

t_start = time()
for step in 1:total_steps

    # A. INJECTION  (bi=1=Input_A, bi=2=Input_B, bi=4=Output, bi=6=Input_0)
    if Input_A[step] > 0.0
    ...
    if Input_B[step] > 0.0
        net.nodes[3].integrator.u[d_inj] = max(0, round(Int, Input_B[step] + randn() * spike_amp_sigma))
        BioKan.notify_bacterium!(net.nodes[3])
    elseif input_silent_O && O_silent_B[step] > 0.0
        net.nodes[3].integrator.u[o_diff_idx] = amp_O
        BioKan.notify_bacterium!(net.nodes[3])
    end
    if Output_forced[step] > 0.0
        net.nodes[6].integrator.u[d_inj] = max(0, round(Int, Output_forced[step] + randn() * spike_amp_sigma))
        BioKan.notify_bacterium!(net.nodes[6])
    elseif output_silent_O && !in_freeze && O_silent_output[step] > 0.0   # output attendu=0 → émet O
        net.nodes[6].integrator.u[o_diff_idx] = amp_O
        BioKan.notify_bacterium!(net.nodes[6])
    end
    if Input_0[step] > 0.0
        net.nodes[8].integrator.u[d_inj] = max(0, round(Int, Input_0[step] + randn() * spike_amp_sigma))
    ...
    if freeze_idx <= length(freeze_intervals)
        ...
        if !in_freeze && t_now >= fi.t_start
            ...
            if freeze_s_mean && !isempty(train_mask)
                # Moyenne de S sur les n_s_mean derniers epochs d'entraînement
                n_avg = min(n_s_mean * 4, length(train_mask))   # 4 combos par epoch
    ...
    for (bi, b) in enumerate(valid_nodes)
        ...
        # Relancer le compte à rebours dès que O est présent (O vient uniquement de diffusion)
        if O_val >= 1.0
        ...
        if M_val >= M_thr_spike && !is_o_inhibited && refrac_counter[bi] == 0 && emit_counter[bi] == 0
            can_spike = (in_freeze && !(bi in inputs_only)) ||
                        ...
                        (bi == 3 && Input_B[step]       > 0.0) ||
                        (bi == 6 && Output_forced[step] > 0.0) ||
                        (bi == 8 && Input_0[step]       > 0.0)
            if can_spike
                ...
                emit_Eint_counter[bi]  = steps_spike   # durée fixe, pas de sigma
                emit_C_counter[bi]     = steps_spike   # durée fixe, pas de sigma
        ...
        if emit_counter[bi] > 0
            if is_o_inhibited
                ...
                b.integrator.u[d_diff_idx]   = (bi == 6) ? 0 : emit_amp_D_vec[bi]   # B6 n'émet pas D_diff (enseignant, pas signalant)
                b.integrator.u[e_diff_idx]   = emit_E_counter[bi] > 0 ? amp_E_ext_diff : 0
                b.integrator.u[c_diff_idx]   = (bi == 6 || (input_silent_C && (bi == 1 || bi == 3))) ? 0 : (emit_C_counter[bi] > 0 ? amp_C_diff : 0)
                ...
                if M_val >= M_thr_O && !(bi in forced_only)
                    ...
                    b.integrator.u[e_int_idx_sp] = 0   # O inhibe l'apprentissage du producteur
                end
                emit_counter[bi]      -= 1
    ...
    # E. TRANSPORT (D/E/C avec délai 150s, O instantané)
    ...
    for (bi, b) in enumerate(valid_nodes)
        ...
        # Relancer o_inhib_counter dès que O_diff est reçu (sans attendre la section C)
...
# --- S[B5=Junction] par trial d'entraînement ---
println("\n--- S[Junction=B5] fin de chaque trial d'entraînement ---")
...
println("\n  Résumé S[B5] moyen par combo (entraînement) :")
...
# --- Rapport XOR : Output (B6) spike-t-il aux bons moments ? ---
# Un spike est "validé" seulement si O_B6 reste < O_veto dans les t_veto secondes suivantes
println("\n--- Rapport $(logical_gate) (B6=Output) ---")
combo_labels = Dict(:A_only => "A seul", :B_only => "B seul", :AB => "A+B  ", :none => "aucun")
O_veto = 3.0      # seuil O au-delà duquel le spike est annulé rétrospectivement
t_veto = 3000.0   # fenêtre post-spike pour vérifier O (s)

# Collecte pour fichier résultats
...
# Affiche les spikes annulés par O : marqueurs ▼ regroupés (1 max par bin de 2*t_refrac)
function add_cancellations!(p, can_log, y_marker; bin_size=2*t_refrac)
    isempty(can_log) && return
    t_max = maximum(tp)
    # déduplique par bins temporels pour ne pas saturer
    last_t = -Inf
    first_added = false
...
plot!(p_input, tp_full[1:n_tp], in0_ds[1:n_tp], label="Input_0 (B8, none)", color=:gray,   lw=1.5)
...
bac_colors = [:blue, :dodgerblue, :orange, :darkorange, :green, :crimson, :purple, :gray]
...
# --- Plot E_int / E_ext : un panel par bactérie, majoré à 100 ---
panels_E = []
for bi in 1:n_bac
    ei_trace = downsample_max(Float64.(history[:, bi, idx_sp[:E_int]]), plot_stride)
    ee_trace = downsample_max(Float64.(history[:, bi, idx_sp[:E_ext]]), plot_stride)
...
batch_id  = length(ARGS) >= 2 ? ARGS[2] : ts_str   # job array ID si dispo, sinon timestamp
out_dir   = joinpath(PROJECT_ROOT, "outputs", "xor_$(batch_id)", gate_str)
mkpath(out_dir)
path_dmi  = joinpath(out_dir, "test_xor_$(ts_str)_DMI.png")
path_zoom = joinpath(out_dir, "test_xor_$(ts_str)_zoom.png")
path_O    = joinpath(out_dir, "test_xor_$(ts_str)_O.png")
path_E    = joinpath(out_dir, "test_xor_$(ts_str)_E.png")
...
p_net = plot(title = "Topologie du réseau (Y)",
...
# --- S moyen et final pendant la phase d'entraînement (panel texte) ---
...
# --- M de toutes les bactéries pendant la phase de test ---
...
path_test = joinpath(out_dir, "test_xor_$(ts_str)_test.png")
savefig(fig_test, path_test)
println("  $path_test")

# --- Fichier résultats texte ---
path_results = joinpath(out_dir, "test_xor_$(ts_str)_results.txt")
open(path_results, "w") do io
    println(io, "=== Résultats $(logical_gate) — $(ts_str) ===\n")
...
# --- Fichier summary global (une ligne par porte, dans le dossier parent) ---
summary_dir  = joinpath(PROJECT_ROOT, "outputs", "xor_$(batch_id)")
path_summary = joinpath(summary_dir, "summary.csv")
path_lock    = joinpath(summary_dir, ".summary.lock")

# Attendre que le lock soit libre (max 60s)
t_lock = time()
