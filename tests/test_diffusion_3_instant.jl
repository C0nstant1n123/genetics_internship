...
circuit_input, p_defaults_vec_input = create_genetic_hill_repeter_input(:node_generic)   # could be create_genetic_simple_circuit_input_integrated
circuit_output, p_defaults_vec_output = create_genetic_hill_repeter_output(:node_generic)
...
k_c = 0.001
#set_param!(params_dict_input, :k_create_X, 0.1)
#set_param!(params_dict_input, :k_deg_X, 0.001)

println(params_dict_input)

#  Build Bacteries
u0_dict_raw_source = Dict(:X => 100.0, :Y => 0.0, :Y_trans => 0.0, :Z => 0.0, :mRNA => 0.0)
u0_dict_raw_receiv = Dict(:X => 0.0, :Y => 0.0, :Y_trans => 0.0, :Z => 800.0,  :mRNA => 0.0)
...
labels = ["X (input)" "Y (protein)" "Y_trans (secreted)" "Z (output)"]

p1 = plot(times, history_all[:, 1, 1:4], title="Bacteria 1 (Source)", label=labels, color=colors, lw=2)
p2 = plot(times, history_all[:, 2, 1:4], title="Bacteria 2 (Receiver)", label=labels, color=colors, lw=2)
...
max_lag = 500000  # Valeur maximale (ex: 20, ajustez basé sur la taille de vos données)
step = 100

lags = min_lag:step:max_lag
mutual_infos = Float64[]  # Array typé pour stocker les MI (plus performant)
for lag in lags 
    len = size(history_all[:, 1, 1])[1]  # Taille en Int64
    ...
    X = history_all[1:(len - lag + 1), 1, 1]  # Premiers éléments de X
    Y = history_all[lag:len, 2, 4]            # Éléments décalés de Y (même taille)
...
p4 = plot(lags .* dt, 
          mutual_infos,
          xlabel = "Time lag (s)",
          ylabel = "Mutual Information (bits/10sec)",
