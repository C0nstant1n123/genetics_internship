...
species_names = [:X]
circuit_input, p_defaults_vec_input = create_simple_death_birth_model(:node_generic)

params_dict_input = Dict(parameters(circuit_input) .=> p_defaults_vec_input)

set_param!(params_dict_input, :k_create, 0.001)
set_param!(params_dict_input, :k_deg, 2.9e-5)
